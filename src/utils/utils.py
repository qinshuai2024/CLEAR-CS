...
try:
    from model.reranker import QueryEncoderWithExperts
except Exception:
    QueryEncoderWithExperts = None
...
def get_optimizer_model_gate(args, model: nn.Module, gate_network: nn.Module, weight_decay: float = 0.0) -> torch.optim.Optimizer:
    """
    Create joint optimizer (model parameters + gate network parameters)
    
    Args:
        args: Training argument object, should contain:
            - learning_rate: base learning rate
            - adam_epsilon: Adam epsilon parameter
        model: Main model (ANCE_WithAdapters instance)
        gate_network: Adaptive gate network
        weight_decay: Weight decay coefficient
    
    Returns:
        Configured AdamW optimizer
    """
    # Define parameter name patterns that don't need weight decay
    no_decay = ['bias', 'LayerNorm.weight']
    
    # 1. Process model parameter grouping
    model_params = [
        {
            'params': [p for n, p in model.named_parameters() 
                      if not any(nd in n for nd in no_decay)],
            'weight_decay': weight_decay,
            'lr': args.learning_rate  # Use base learning rate
        },
        {
            'params': [p for n, p in model.named_parameters() 
    ...
    # 2. Process gate network parameter grouping
    gate_params = [
        {
            'params': [p for n, p in gate_network.named_parameters() 
                      if not any(nd in n for nd in no_decay)],
            'weight_decay': weight_decay,
            'lr': args.learning_rate * 2  # Gate network usually needs larger learning rate
        },
        {
            'params': [p for n, p in gate_network.named_parameters() 
                      if any(nd in n for nd in no_decay)],
    ...
    optimizer_grouped_parameters = model_params + gate_params
    
    # Create optimizer
    optimizer = AdamW(
        optimizer_grouped_parameters,
        lr=args.learning_rate,  # Base learning rate (can be overridden by each parameter group)
        eps=args.adam_epsilon # Consistent with original BERT optimization settings
    )
    
    return optimizer


def load_model_expert(model_dir):
    if QueryEncoderWithExperts is None:
        raise ImportError("QueryEncoderWithExperts is not available in the current codebase")
    # 1. Load configuration
    with open(os.path.join(model_dir, "model_config.json"), "r") as f:
        config = json.load(f)
    
    # 2. Initialize empty model
    model = QueryEncoderWithExperts(
