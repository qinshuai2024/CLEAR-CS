...
from model.ance import load_model
import toml
# from IPython import embed
from utils.utils import check_dir_exist_or_build
import gzip
...
def PassagePreprocessingFn(args, line, tokenizer, title = True):
    # topiocqa tiltle = True
    # title = True
    title = False
    line = line.strip()
        ...
        try:
            ...
            if title == True:
                if len(line_arr) < 3:
                    ...
                    title_text = line_arr[2].rstrip().replace(' [SEP] ', ' ')
                p_text = title_text + ' ' + line_arr[1].rstrip()
            else:
