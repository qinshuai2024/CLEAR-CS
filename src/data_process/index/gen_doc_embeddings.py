...
# Add parent's parent directory to Python path
# Get absolute path of current file
current_dir = os.path.dirname(os.path.abspath(__file__))
# Go up four levels to project_root
...
from src.model.ance import load_model
...
def GetProcessingFn(args, query=False):
    def fn(vals, i):
        ...
        all_input_ids_a = torch.tensor(
            np.array([f[1] for f in passage_collection]), dtype=torch.int32)  # Note here
        all_attention_mask_a = torch.tensor(
...
def InferenceEmbeddingFromStreamDataLoader(
    ...
    tmp_n = 0
    expect_per_block_passage_num = 2500000 
    block_size = expect_per_block_passage_num // eval_batch_size
    block_id = 0
    total_write_passages = 0

    # total = 数据集里的 passage 数
    # total_passages = len(train_dataloader.dataset)
    # total_passages = 25700592
    total_passages = 35842584

    with tqdm(total=total_passages, desc="Inferencing (passages)", disable=args.disable_tqdm) as pbar:
        for batch in train_dataloader:
            ...
            if tmp_n % block_size == 0:
                embedding = np.concatenate(embedding, axis=0)
                embedding2id = np.concatenate(embedding2id, axis=0)
                emb_block_path = os.path.join(args.data_output_path, f"passage_emb_block_{block_id}.pb")
                with open(emb_block_path, 'wb') as handle:
                    pickle.dump(embedding, handle, protocol=4)
                embid_block_path = os.path.join(args.data_output_path, f"passage_embid_block_{block_id}.pb")
                with open(embid_block_path, 'wb') as handle:
                    pickle.dump(embedding2id, handle, protocol=4)
                total_write_passages += len(embedding)
                block_id += 1
...
# Note: Need to modify total_passages in the code
# Example usage:
# CUDA_VISIBLE_DEVICES=0 nohup python gen_doc_embeddings.py --config gen_doc_embeddings.toml > logs/embedding_generation.out 2>&1 &
